...
class PowerMonitorApp:
    ...
    def _build_gui(self) -> None:
        ...
        # # PDA50B2
        # tk.Label(display_frame, text="PDA50B2 Power (mW):", font=("Arial", 1, "bold")).grid(
        #     row=0,
        #     column=0,
        #     sticky="e",
        #     padx=5,
        ...
        # tk.Label(display_frame, text="Voltage (V):",font=("Arial", 1, "bold")).grid(
        #     row=1,
        #     column=0,
        #     sticky="e",
        #     padx=5,
        #     pady=2,
        # )
        # self.lbl_pda50b2_voltage = tk.Label(display_frame, text="--",font=("Arial", 1, "bold"))
        # self.lbl_pda50b2_voltage.grid(row=1, column=1, sticky="w", padx=5, pady=2)

        # PDA100A2
        tk.Label(display_frame, text="Excitation Power (mW):", font=("Arial", 18, "bold")).grid(
            row=2,
            column=0,
            sticky="e",
            padx=5,
        ...
        tk.Label(display_frame, text="PDA100A2 Voltage (V):", font=("Arial", 18, "bold")).grid(
            row=3,
            column=0,
            sticky="e",
            padx=5,
        ...
        self.lbl_pda100a2_voltage = tk.Label(display_frame, text="--", font=("Arial", 18, "bold"))
    ...
    def init_daq(self) -> None:
        ...
        try:
            ...
            test_ctrl = PhotodetectorController(device, channel2=ch2)
            v1, v2 = test_ctrl.read_both_detectors()

            # If we get here, DAQ is working; keep a controller instance.
            self.detector_ctrl = PhotodetectorController(device, channel2=ch2)
            self.lbl_status.config(
                #text=f"DAQ: Initialized ({device}, {ch1}, {ch2})",
                text=f"DAQ: Initialized ({device},  {ch2})",
                fg="green",
            )
    ...
    # redefine with quadratic function
    def _voltage_to_mw(self, voltage: float) -> float:
        """Convert voltage (V) to power (MW) using fixed calibration."""
        a = 7.5461e-4
        b= 0.64023
